...
int main()
{
  cout << "Startar med att l‰sa frÂn en fil." << endl;
  ...
  while (operation != 0)
    {
      ...
      cout << "0. Avsluta. Alla transaktioner sparas pÂ fil." << endl;
      cout << "1. Skriv ut information om alla transaktioner." << endl;
      cout << "2. L‰s in en transaktion frÂn tangentbordet." << endl;
      ...
      switch (operation)
        {
          ...
          case 2:
            {
              Transaktion transaktion;
              cout << "Ange transaktion i fˆljande format" << endl;
              transaktion.skrivTitel(cout);
          ...
          case 3:
            {
              cout << "Den totala kostnanden fˆr resan var "
...
Transaktion *ny = 0;
    ny = new Transaktion[antal_transaktioner+1];

    for ( int i = 0; i < antal_transaktioner; i++ )
	{
	  ny[i] = transaktioner[i];
	}
      ny[antal_transaktioner++] = t;
      delete [] transaktioner;
      transaktioner = ny;
   }
...
bool check;
    for(int i = 0; i < antal_transaktioner; i++){
        ...
        if(check == 0){
               betala = liggerUteMed(nam);
               skuld = arSkyldig(nam);
            Person nya(nam,betala , skuld);
...
{
    os << namn << " ligger ute med: " << hamtaBetalat() <<" och ‰r skyldig: " << hamtaSkyldig() << " " ;
    if(hamtaSkyldig() < hamtaBetalat())
    {
        os << hamtanamn() << " Skall ha " << hamtaBetalat() - hamtaSkyldig() << " frÂn potten!" << endl;
    }
    else
    {
        os << hamtanamn() << " Skall l‰gga " << hamtaSkyldig() - hamtaBetalat() << " till potten!" << endl;
    }
}
...
void PersonLista::laggTill(Person ny_person)
{
    ...
    Person *nyp = 0;
    nyp = new Person[antal_personer +1];
...
os << "-----------------------------------------------------------" <<endl;
    ...
    if (summaSkyldig() == summaBetalat())
        os << "\nDe belopp personer l‰gger till potten ‰r lika me de som andra ta frÂn potten!";
}
...
double PersonLista::summaBetalat()
{
 ...
 bool PersonLista::finnsPerson(const string& namn){  //oklart om maxpersoner borde anv‰ndas ist‰llet
 bool check = false;
...
Transaktion& Transaktion::operator=( const Transaktion& t)
{
 ...
 PersonLista& PersonLista::operator=( const PersonLista& t){
     if (this != &t)
    {
        antal_personer = t.antal_personer;
        personer = new Person[antal_personer];
        for (int i = 0; i < antal_personer; i++){
            personer[i] = t.personer[i];
        }
    }
    return *this;
 }
